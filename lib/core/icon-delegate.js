...
class IconDelegate{
	 ...
	 */
	getClasses(){
		const colourMode = atom.config.get("file-icons.onChanges") && !this.resource.vcsStatus
			? null
			: FileIcons.ui.colourMode;
	...
	getFallbackClasses(){
		...
		if(resource.type & EntityType.DIRECTORY)
			...
			const classes = atom.config.get("file-icons.defaultIconClass");
			return classes
	...
	getCurrentIcon(){
		if(this.currentIcon)
			...
			FileIcons.strategies.query(this.resource);
	...
	deserialise(){
		...
		const {iconTables, storage, strategies} = FileIcons;
		...
		setImmediate(() => strategies.query(this.resource));
	}
	
	
	serialise(){
		if(!FileIcons.storage.locked){
			const path = normalisePath(this.resource.path);
			const icon = this.currentIcon;
			
			if(icon)
				FileIcons.storage.setPathIcon(path, {
				...
				FileIcons.storage.deletePathIcon(path);
		}
	}
	...
	set master(input){
		...
		const currentIcon  = originalIcon;
