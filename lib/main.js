...
const FileIcons = {
	...
	async activate(state){
		...
		await (this.loadPromise = Promise.all([
			this.setupStorage(state),
	...
	serialize(){
		return this.storage
			? this.storage.serialise()
			: {};
	},
	
	
	/**
	 * Check whether a named component of the package has finished loading.
	 *
	 * @internal
	 * @param {String} state - Lowercased suffix of a `LOADED_*` constant.
	 * @return {Boolean}
	 */
	hasLoaded(state){
		switch(String(state).toLowerCase()){
			case "all":        return !!(this.loadState & ACTIVATED);
			case "cache":      return !!(this.loadState & LOADED_CACHE);
			case "options":    return !!(this.loadState & LOADED_OPTIONS);
			case "ui":         return !!(this.loadState & LOADED_UI);
			case "icons":      return !!(this.loadState & LOADED_ICONS);
			case "strategies": return !!(this.loadState & LOADED_STRATEGIES);
		}
	 ...
	 * @public
	 * @returns {Disposable}
	 *    A Disposable that destroys the {@link IconNode} when disposed of.
	 *    Authors are encouraged to do so once the element is no longer needed.
	 * 
	...
	addIconToElement(element, path, typeHints = {}){
		...
		if(element){
			...
			if(icon && !icon.destroyed && this.iconDisposables.has(icon))
			...
			// Invalid input: don't break the user's workflow, but do emit an error message
				...
				let node = null;
				disposable = new Disposable(() => {
					this.iconDisposables.delete(node);
					if(!node) return;
					node.removeClasses();
					node.destroy();
				});
				this.loadPromise.then(() => {
					...
					const IconNode = require("./core/icon-node.js");
	...
	handleResource(resource){
		...
		const IconDelegate = require("./core/icon-delegate.js");
		const icon = new IconDelegate(resource);
		resource.icon = icon;
		const disposable = resource.onDidDestroy(() => {
			icon.destroy();
			this.disposables.delete("icons", disposable);
			resource.icon = null;
		});
		this.disposables.add("icons", disposable);

		// TODO: Add `.inode` property to Resource class
		if(resource.stats && resource.stats.ino){
			const inode = resource.stats.ino;
			this.storage.setPathInode(resource.path, inode);
		}

		if(resource.type & EntityType.SYMLINK){
			const disposable = resource.onDidChangeRealPath(({to}) => {
	...
	/**
	 * Return a reference to the package's service handler.
	 *
	 * @see {#addIconToElement}
	 * @return {Function}
	 * @public
	 */
	provideService(){
	...
	/**
	 * Provide an object which defines the legacy service handler.
	 *
	 * @deprecated
	 * @see {@link #iconClassForPath}
	 * @return {Object}
	 * @public
	 */
	provideLegacyService(){
		const fn = this.iconClassForPath.bind(this);
		return {iconClassForPath: fn};
	},
	...
	resetCache(){
		if(this.storage.locked)
			...
			this.disposables.dispose("storage");
	...
	/**
	 * Initialise {@link StrategyManager}.
	 * @internal
	 */
	async setupStrategies(){
		const StrategyManager = require("./core/strategy-manager.js");
		...
		this.disposables.add("service",
			...
			// Handle a newly-registered filesystem resource
			FileSystem.observe(this.handleResource),
	...
	/**
	 * Initialise {@link IconTables} and expand compiled icon definitions.
	 * @internal
	 */
	async setupTables(){
		const IconTables = require("./core/icon-tables.js");
		const sourcePath = require.resolve("./compiler/.icondb.js");
		this.iconTables  = new IconTables(sourcePath);
		await this.iconTables.loadPromise;
		this.readyState |= LOADED_ICONS;
	},
	 ...
	 */
	async setupOptions(){
		this.disposables.add("options", atom.config.observe("file-icons.coloured", enabled => {
	 ...
	 */
	async setupCommands(){
		this.disposables.add("commands", atom.commands.add("atom-workspace", {
			"file-icons:clear-cache":    () => this.resetCache(),
			"file-icons:toggle-colours": () => {
				const coloured = atom.config.get("file-icons.coloured");
				atom.config.set("file-icons.coloured", !coloured);
			},
			"file-icons:toggle-changed-only": () => {
				const onChanged = atom.config.get("file-icons.colourChangedOnly");
				atom.config.set("file-icons.colourChangedOnly", !onChanged);
			},
			"file-icons:toggle-tab-icons": () => {
				const tabIcons = atom.config.get("file-icons.tabPaneIcon");
				atom.config.set("file-icons.tabPaneIcon", !tabIcons);
			},
		}));
	},
...
// Bind each of the package's methods to itself
for(const key of Object.keys(FileIcons)){
	const fn = FileIcons[key];
	if("function" === typeof fn)
		FileIcons[key] = fn.bind(FileIcons);
}
